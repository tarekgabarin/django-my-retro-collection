...
class GameTests(TestCase):
    ...
    def test_create_game(self):
        ...
        new_game.tags_for_game.set(
            [self.action_tag, self.three_dimensional_tag, self.multi_player_tag]
        )
        new_game.players_that_own_game.set([])

        self.assertEqual(new_game.title, new_game_title)

        games_that_are_3d_and_multiplayer = list(
            Game.objects.filter(tags_for_game__in=[self.multi_player_tag])
            .filter(tags_for_game__in=[self.three_dimensional_tag])
            .values()
        )
        self.assertEqual(2, len(games_that_are_3d_and_multiplayer))

        expected_games_from_3d_multiplayer_query = sorted(
            [self.quake_game.title, new_game_title]
        )
        actual_games_from_3d_multiplayer_query = sorted(
            [obj["title"] for obj in games_that_are_3d_and_multiplayer]
        )
        self.assertEqual(
            expected_games_from_3d_multiplayer_query,
            actual_games_from_3d_multiplayer_query,
        )
    ...
    def test_update_game_info(self):
        ...
        Game.objects.filter(title=new_game_title).update(
            publisher_of_game=self.capcom_publisher
        )
        game_obj_to_modify = Game.objects.get(title=new_game_title)
        game_obj_to_modify.game_consoles_released_on.set(
            [self.psx_console, self.windows_console]
        )

        # Check if the update we did works
        updated_entry = Game.objects.get(title=new_game_title)
        number_of_consoles_for_updated_entry = len(
            list(updated_entry.game_consoles_released_on.all())
        )
    ...
    def test_get_games_for_specified_consoles(self):
        ...
        games_on_dreamcast = list(
            Game.objects.filter(
                game_consoles_released_on__in=[self.dc_console]
            ).values()
        )
        ...
        games_on_both_psx_and_windows = list(
            Game.objects.filter(game_consoles_released_on__in=[self.windows_console])
            .filter(game_consoles_released_on__in=[self.psx_console])
            .values()
        )
        expected_result_both_psx_windows = sorted(
            [resident_evil_obj.title, doom_obj.title]
        )
        actual_result_both_psx_windows = sorted(
            [obj["title"] for obj in games_on_both_psx_and_windows]
        )

        self.assertEqual(
            expected_result_both_psx_windows, actual_result_both_psx_windows
        )
        ...
        games_on_psx_OR_windows = list(
            Game.objects.filter(
                game_consoles_released_on__in=[self.psx_console, self.windows_console]
            )
            .values()
            .distinct()
        )
        expected_result_psx_OR_windows = sorted(
            [
                final_fantasy_obj.title,
                chrono_trigger_obj.title,
                doom_obj.title,
                quake_obj.title,
                resident_evil_obj.title,
            ]
        )
        actual_result_psx_OR_windows = sorted(
            [obj["title"] for obj in games_on_psx_OR_windows]
        )
    ...
    def test_get_games_from_specified_publisher(self):
        ...
        games_published_by_capcom = list(
            Game.objects.filter(publisher_of_game=self.capcom_publisher).values()
        )
        expected_result_capcom_games_query = sorted(
            [megaman_obj.title, ghouls_ghosts_obj.title, resident_evil_obj.title]
        )
        actual_result_capcom_games_query = sorted(
            [obj["title"] for obj in games_published_by_capcom]
        )

        self.assertEqual(
            expected_result_capcom_games_query, actual_result_capcom_games_query
        )
        self.assertEqual(3, len(actual_result_capcom_games_query))

        konami_games_released_on_genesis = list(
            Game.objects.filter(
                publisher_of_game=self.konami_publisher,
                game_consoles_released_on__in=[self.genesis_console],
            ).values()
        )
        expect_result_konami_sega_query = [contra_obj.title]
        unexpected_result_konami_sega_query = sorted(
            [castlevania_obj.title, contra_obj.title]
        )
        actual_result_konami_sega_query = sorted(
            [obj["title"] for obj in konami_games_released_on_genesis]
        )

        self.assertEqual(
            expect_result_konami_sega_query, actual_result_konami_sega_query
        )
        self.assertEqual(1, len(konami_games_released_on_genesis))
        self.assertNotEqual(
            unexpected_result_konami_sega_query, actual_result_konami_sega_query
        )
    ...
    def test_get_list_of_games_based_on_tags(self):
        ...
        games_under_general_rpg_category = list(
            Game.objects.filter(tags_for_game__in=[self.rpg_tag]).values()
        )
        expected_result_general_rpgs = sorted(
            [final_fantasy_obj.title, shining_force_obj.title, chrono_trigger_obj.title]
        )
        actual_result_general_rpgs = sorted(
            [obj["title"] for obj in games_under_general_rpg_category]
        )
        ...
        games_under_strategy_subcategory = list(
            Game.objects.filter(tags_for_game__in=[self.rpg_tag])
            .filter(tags_for_game__in=[self.strategy_tag])
            .values()
        )
        expected_result_strategy_rpg = sorted(
            [final_fantasy_obj.title, shining_force_obj.title]
        )
        unexpected_result_strategy_rpg = sorted(
            [final_fantasy_obj.title, shining_force_obj.title, chrono_trigger_obj.title]
        )
        actual_result_strategy_rpg = sorted(
            [obj["title"] for obj in games_under_strategy_subcategory]
        )
        ...
        games_under_general_shooter_category = list(
            Game.objects.filter(tags_for_game__in=[self.shooter_tag]).values()
        )
        ...
        expected_result_general_shooter = sorted(
            [doom_obj.title, quake_obj.title, contra_obj.title]
        )
        actual_result_general_shooter = sorted(
            [obj["title"] for obj in games_under_general_shooter_category]
        )
        ...
        games_under_fps_subcategory = list(
            Game.objects.filter(tags_for_game__in=[self.shooter_tag])
            .filter(tags_for_game__in=[self.fps_tag])
            .values()
        )

        expected_result_fps = sorted([doom_obj.title, quake_obj.title])
        unexpected_result_fps = sorted(
            [doom_obj.title, quake_obj.title, contra_obj.title]
        )
        actual_result_fps = sorted(
            [obj["title"] for obj in games_under_fps_subcategory]
        )
        ...
        action_games_that_are_2d = list(
            Game.objects.filter(tags_for_game__in=[self.action_tag])
            .filter(tags_for_game__in=[self.two_dimensional_tag])
            .values()
        )
        expected_result_2d_action = sorted(
            [
                castlevania_obj.title,
                megaman_obj.title,
                ghouls_ghosts_obj.title,
                contra_obj.title,
            ]
        )
        actual_result_2d_action = sorted(
            [obj["title"] for obj in action_games_that_are_2d]
        )
    ...
    def test_adding_and_removing_games_to_players_collection(self):
        rpg_fan = get_user_model().objects.create_user(
            email="rpg_fan_69@testing.com", password="Testing123/!/"
        )
        ...
        rpg_fans_game_collection_after_adding_games = (
            get_user_model()
            .objects.get(email="rpg_fan_69@testing.com")
        ...
        rpg_fans_game_collection_after_removing_games = (
            get_user_model()
            .objects.get(email="rpg_fan_69@testing.com")
            .games_owned_by_player.all()
        )
        ...
        self.assertNotIn(
            shining_force_obj, rpg_fans_game_collection_after_removing_games
        )
